...
import { mockEvents, mockDeals, mockCommunityLeaders, featuredBusinesses } from '@/lib/mock-data';
...
export default function HomePage() {
  ...
  if (!isMounted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-background p-4">
        ...
        {/* Simplified PWA-like splash screen for initial load */}
  ...
  const hotDeals = mockDeals.filter(deal => (deal.confidence ?? 0) > 0.7).slice(0, 3);
  const communityLeadersToShow = mockCommunityLeaders.slice(0, 3);
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <section className="relative py-20 md:py-32 bg-gradient-to-br from-primary/80 via-primary to-secondary/80 text-primary-foreground overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          {/* Decorative background image - subtle Tampa skyline or abstract */}
          <Image
            src="https://placehold.co/1920x1080.png" // Replace with an actual subtle background image
      ...
      {/* Tampa Tip Section */}
      <section className="py-10 bg-card border-b border-t border-border">
      ...
      {/* Featured Businesses / Ad Slideshow */}
      <AdSlideshow />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {[
              ...
              { name: "Nightlife", icon: Sparkles, href: "/events?category=Nightlife", "data-ai-hint": "city nightlife" }, // Assuming Nightlife is an event category
        ...
        {communityLeadersToShow.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8">
              {communityLeadersToShow.map(leader => <PersonCard key={leader.id} leader={leader} />)}
            </div>
          </section>
        )}
      </div>
